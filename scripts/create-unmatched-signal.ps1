...
function Convert-ConnectionStringToPsqlArgs {
    ...
    if ($normalized -match "\s+\w+\s*=" -and $normalized -notmatch ";") {
        return @($normalized)
    }
    ...
    foreach ($part in $parts) {
        ...
        elseif ([string]::IsNullOrWhiteSpace($dbHost)) {
            $dbHost = $segment
        }
    }
...
if (-not [Guid]::TryParse($CampaignId, [ref]([Guid]::Empty))) {
...
WITH campaign_context AS (
    SELECT
        ...
        COALESCE(
            ...
            NULLIF(
                regexp_replace(
                    COALESCE(NULLIF(campaign."LandingPageURL", ''), 'unknown'),
                    '^https?://([^/?#]+).*$',
                    '\1'
                ),
                ''
...
inserted_event AS (
    ...
    SELECT
        ...
        gen_random_uuid()::text,
        'local-unmatched-signal:' || context."TrackerId"::text || ':' || gen_random_uuid()::text,
        NOW(),
        NOW(),
        'Unmatched',
        true,
        ...
        NULL,
        NULL,
        md5(context."TrackerId"::text || ':' || NOW()::text || ':' || random()::text),
        'local_unmatched_signal',
        NOW(),
        NOW()
    FROM campaign_context context
...
$tempSqlPath = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath ("create_unmatched_signal_{0}.sql" -f [Guid]::NewGuid().ToString("N"))
...
Write-Host "Creating unmatched signal for campaign: $CampaignId"
Write-Host ("Using psql target args: {0}" -f ($connectionArgs -join " "))

try {
    & $PsqlPath @arguments
    if ($LASTEXITCODE -ne 0) {
        throw "psql exited with code $LASTEXITCODE while creating unmatched signal."
    }
}
finally {
